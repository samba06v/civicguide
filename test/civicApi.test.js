...
import { buildPollingMapLink, buildVoterInfoUrl, normalizeVoterInfo } from '../src/utils/civicApi.js';
...
test('buildPollingMapLink creates Google Maps link', () => {
  const mapLink = buildPollingMapLink('1 Infinite Loop, Cupertino');
  assert.match(mapLink, /google\.com\/maps\/search/);
  assert.match(mapLink, /1%20Infinite%20Loop%2C%20Cupertino/);
});
...
test('normalizeVoterInfo returns stable defaults', () => {
  ...
  assert.deepEqual(result.electionOfficials, []);
});
